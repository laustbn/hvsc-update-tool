...
PathCreator::PathCreator(const char* inPath) : maxPathLen(maxCharBufferLen) {
  path = new char[maxPathLen];
  path[0] = 0;
  if (inPath != 0) strcpy(path, inPath);
}
...
const char* PathCreator::get(void) { return path; }

char* PathCreator::getWritable(void) { return path; }

void PathCreator::empty(void) { path[0] = 0; }
