...
PathSplitter::PathSplitter(const std::filesystem::path path) : path_(path) {
...
bool PathSplitter::nextFile() {
  ...
  it++;
  if (it == last || it == path_.end()) return false;

  return true;
}

bool PathSplitter::isLastFile() {
  if (it == path_.end()) return false;
