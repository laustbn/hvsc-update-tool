...
TextFile::~TextFile() {
  close();
  if (lineBuf != 0) delete[] lineBuf;
  if (parseBuf != 0) delete[] parseBuf;
}
...
const char* TextFile::getLineBuf() const { return lineBuf; }

const unsigned long TextFile::getLineNum() const { return lineNum; }

int TextFile::getLineLen() const { return lineLen; }
...
bool TextFile::createParseCopy() {
  ...
  for (int i = 0; i < lineLen;
       i++) {  // The code here uses extended ASCII characters > 127.
