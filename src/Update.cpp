...
static const char DIR_SEPARATOR[] = {PATH_SEPARATOR, 0};
static const char PARENT_DIR[] = "..";
static const char PARENT_DIR_PREFIX[] = {'.', '.', PATH_SEPARATOR, 0};
...
void logError(ofstream& errorFile, const char* lineContent,
              const char* errorMessage, int iLineNum, mode_type mode,
              int& errorCount);
...
inline void appExit(int returnValue) {
  if (fileCopyBuffer != 0) delete[] fileCopyBuffer;
...
int main(int, char* argv[]) {
  ...
  for (auto const& f : fileNames) {
    sprintf(hvsidsFileNameTmp.getWritable(), "%s/%s", DOCUMENTS_DIR, f);
  ...
  for (updateNum = 1; updateNum <= maxUpdateNum; updateNum++) {
    ...
    if (updateNum <= 99) {
      sprintf(updateFileNameCat.getWritable(), "%s/Update%02d.hvs", UPDATE_DIR,
              updateNum);
      sprintf(errorsFileName.getWritable(), "Errors%02d.hvs", updateNum);
    } else {
      sprintf(updateFileNameCat.getWritable(), "%s/Update%d.hvs", UPDATE_DIR,
              updateNum);
  ...
  if (lines == 0) {
    cerr << "HVSC update script file ``" << updateFileName.get()
         << "'' is empty." << endl;
    appExit(-1);
  }
  ...
  if (updateNum > 1) {
    ...
    HVSCversion.havePrevScript =
        getHVSCpath(prevUpdateFileName, prevUpdateFileNameCat.get());
  } else {
  ...
  if (!updateFile.open(updateFileName.get())) {
    cerr << endl
         << "Error: Could not open ``" << updateFileName.get() << "''." << endl;
    appExit(-1);
  }
    ...
    switch (mode) {
      ...
      case FLAGS: {
        ...
        if (!getHVSCpath(tmpSource, updateFile.getLineBuf())) {
          ...
          if (!sidFile) {
            logError(errorFile, tmpSource.get(), "Could not load sidtune file.",
      ...
      case DELETEMODE: {
        ...
        if (!getHVSCpath(dest, updateFile.getLineBuf())) {
          ...
          if (isDir(dest.get())) {
            if (filesystem_remove(dest.get()) == false) {
              logError(errorFile, dest.get(), "Could not delete directory.",
                       line, mode, errorCount);
            }
          } else {
            if (remove(dest.get()) != 0) {
              logError(errorFile, dest.get(), "Could not delete file.", line,
                       mode, errorCount);
            }
          }
        }
      ...
      case REPLACE: {
        ...
        if (!getHVSCpath(source, curLine)) {
          logError(errorFile, curLine,
                   "(source error) No such path or permission denied.", line,
                   mode, errorCount);
        ...
        if (updateFile.isBlank()) {
          logError(errorFile, updateFile.getLineBuf(),
                   "Premature end of update Script?", destLine, mode,
                   errorCount);
          break;
        }
        ...
        if (!getHVSCpath(dest, updateFile.getLineBuf())) {
          ...
          if (lastChar == '/' || lastChar == '\\') {
            makeHVSCdir(errorFile, errorCount, destLine,
                        updateFile.getLineBuf());
            // Now this should not return an error.
            if (!getHVSCpath(dest, updateFile.getLineBuf())) {
              logError(errorFile, updateFile.getLineBuf(),
                       "Creation of directory failed.", destLine, mode,
                       errorCount);
              break;
            }
          } else {
        ...
        if (isDir(source.get())) {
          ...
          if (!isDir(dest.get()) || dest.isEmpty()) {
            logError(errorFile, dest.get(), "Destination is not a directory.",
          ...
          while ((entry = readdir(directory)) != NULL) {
            ...
            if (!isDir(sourceFile.get()))  // omit directories
            {
              ...
              if (fileCopy(errorFile, errorCount, line, sourceFile.get(),
                           destFile.get())) {
                if (remove(sourceFile.get()) != 0)
                  logError(errorFile, sourceFile.get(),
                           "Could not remove source file.", line, mode,
                           errorCount);
              }
        ...
        {       // source has platform-specific path
          ...
          if (fileCopy(errorFile, errorCount, line, source.get(),
                       newDestFile.get())) {
            if (remove(source.get()) != 0)
              logError(errorFile, source.get(), "Could not remove source file.",
                       line, mode, errorCount);
          }
        }  // single file
        break;
  ...
  if (errorCount == 0) {
    ...
    fileCopy(errorFile, errorCount, 0, updateFileName.get(), docDest.get());
  }
...
bool getHVSCpath(PathCreator& outPath, const char* path) {
  ...
  if (FINDPATH_DEBUG) cout << "getHVSCpath(), path = " << path << endl;
  ...
  if (FINDPATH_DEBUG) cout << "outPath = " << outPath.get() << endl;
  ...
  do {
    ...
    if (FINDPATH_DEBUG) cout << "found = " << found << endl;
...
bool makeHVSCdir(ofstream& errorFile, int& errorCount, int line,
  ...
  if (!getHVSCpath(tmpDest, hvscPath))  // path not found?
  {
    ...
    if (!myPathSplitter.isGood()) {
      logError(errorFile, hvscPath, "Invalid argument.", line, mode,
               errorCount);
      return false;
    }
    ...
    if (myPathSplitter.isLastFile()) {
      ...
      {
        logError(errorFile, tmpDest.get(), "Path does not exist.", line, mode,
                 errorCount);
        return false;
      }
    ...
    if (!success) {
      logError(errorFile, dest.get(), "Could not create directory.", line, mode,
               errorCount);
    }
    return success;
  } else  // path found
  {
    ...
    if (!success) {
      logError(errorFile, tmpDest.get(),
               "Trying to create directory on top of file.", line, mode,
               errorCount);
    }
    return success;
  }
...
bool fileCopy(ofstream& errorFile, int& errorCount, int line,
...
#endif
  {
    logError(errorFile, inFileName, "Could not open source file.", line, mode,
             errorCount);
    return false;
  }

  if (fileExists(outFileName)) {
    logError(errorFile, outFileName, "Destination file already exists.", line,
             mode, errorCount);
    return false;
  }
...
#endif
  {
    logError(errorFile, outFileName, "Could not open destination file.", line,
             mode, errorCount);
    return false;
  }
  ...
  if (fIn.bad() || fOut.bad() || fOut.fail()) {
    logError(errorFile, outFileName, "Stream error during file copy.", line,
             mode, errorCount);
    fIn.close();
    fOut.close();
    return false;
...
bool isDir(const char* fileName) {
  ...
  if (!openFailed) closedir(directory);
  return !openFailed;
}
