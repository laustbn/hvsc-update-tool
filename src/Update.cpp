...
#define SCSV static constexpr std::string_view

SCSV HVSCcontactEmail = "HVSC Admin <hvsc@c64.org>";

// HVSC top-level directories. Case does not matter because it
// will be determined by the update tool.
SCSV UPDATE_DIR = "update";
//
// HVSC 2.0-post (long file names)
SCSV LONG_DOCUMENTS_DIR = "DOCUMENTS";
SCSV LONG_HUBBARD_ROB_DIR_PRE46 = "Hubbard_Rob";
SCSV LONG_HUBBARD_ROB_DIR = "MUSICIANS/H/Hubbard_Rob";

// The main HVSC doc file is read by the revision check code.
SCSV HVSIDS_TXT = "HVSC.txt";
SCSV OLD_HVSIDS_TXT = "hv_sids.txt";
...
int main(int, char* argv[]) {
  ...
  cout << endl
       << "HVSC Update Tool   Version "
       << HVSC_VERSION_STRING
       << " (" << BUILD_GIT_VERSION << ")" << endl
       << endl;
  ...
  auto DOCUMENTS_DIR = std::string{LONG_DOCUMENTS_DIR};
  auto HUBBARD_ROB_DIR = std::string{LONG_HUBBARD_ROB_DIR};
  ...
  if (foundDocumentsDir)
    cout << path_for_display(documentsPath);
  else
    cout << DOCUMENTS_DIR;
  ...
  for (auto const& f : fileNames) {
    ...
    if (getHVSCpath(hvsidsFileName, hvsidsFileNameTmp)) {
      ...
      while (hvsidsFile.NextLine() && hvsidsFile.GetLineNum() <= 10) {
        ...
        if (rel) {
          found = atohvscver(rel.value(), HVSCversion.found);
          // HVSCversion_found = HVSCversion.found;
          // Also break if parsing failed.
          break;
        }
      }
    }
  ...
  if (hvscvercmp(HVSCversion.required, MAKE_HVSCVER(0, 0)) > 0 &&
    ...
    std::string ver = hvscvertoa(HVSCversion.required);
    ...
    if (hvscvercmp(HVSCversion.found, MAKE_HVSCVER(0, 0)) > 0) {
      ver = hvscvertoa(HVSCversion.found);
      cout << ver;
    } else {
      cout << "CHECK FAILED";
    }
          ...
          // SIDMODEL, FREEPAGES
            ...
            if (!updateFile.NextLine())
              logError(errorFile, tmpSource.string(),
                       "Premature end of update script?",
                       updateFile.GetLineNum(), mode, errorCount);
            sidInfo[0] = updateFile.GetLine();
          }

          mySidTune sidFile(tmpSource.string().c_str(), HVSCversion.found);
          if (!sidFile) {
            logError(errorFile, tmpSource.string(),
  ...
  if (errorCount == 0) {
    ...
    cout << "Update was successful! Update directory is no longer needed."
         << endl
         << endl
         << "P.S. - Check the update file in the ``" << path_for_display(documentsPath)
         << "'' directory for" << endl
