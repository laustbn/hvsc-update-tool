...
static const char text_packeddatacorrupt[] =
    "PowerPacker: Packed data is corrupt";
static const char text_unrecognized[] =
    "PowerPacker: Unrecognized compression method";
static const char text_uncompressed[] =
    "Not compressed with PowerPacker (PP20)";
...
static inline void bytesTOudword() {
  ...
  if (readPtr < sourceBuf) {
    ...
    current =
        readEndian(*readPtr, *(readPtr + 1), *(readPtr + 2), *(readPtr + 3));
}
...
udword ppUncompressedLen() { return outputLen; }

bool ppIsCompressed() { return isCompressed; }
...
bool depp(ifstream& source, ubyte** destRef) {
  ...
  udword eff =
      readEndian(efficiency[0], efficiency[1], efficiency[2], efficiency[3]);
  ...
  do {
    if (ppRead(1) == 0) ppBytes();
    if (writePtr > *destRef) ppSequence();
    if (globalError) {
      ppFreeMem();
      return false;
    }
