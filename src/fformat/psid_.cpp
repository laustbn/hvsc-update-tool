...
bool sidTune::PSID_fileSupport(const void* buffer, udword bufLen) {
  ...
  if (readBEword(pHeader->version) >= 2) {
    ...
    if (flags & PSID_MUS) {  // MUS tunes run at any speed
    ...
    if (flags & PSID_CLOCK_PAL) clock |= SIDTUNE_CLOCK_PAL;
    if (flags & PSID_CLOCK_NTSC) clock |= SIDTUNE_CLOCK_NTSC;
    info.clockSpeed = clock;

    info.sidModel = SIDTUNE_SIDMODEL_UNKNOWN;
    if (flags & PSID_SIDMODEL_6581) info.sidModel |= SIDTUNE_SIDMODEL_6581;
    if (flags & PSID_SIDMODEL_8580) info.sidModel |= SIDTUNE_SIDMODEL_8580;
    // added for v3
    if (info.version >= 3) {
      if (flags & PSID_SIDMODEL2_6581) info.sidModel |= SIDTUNE_SIDMODEL2_6581;
      if (flags & PSID_SIDMODEL2_8580) info.sidModel |= SIDTUNE_SIDMODEL2_8580;
      info.reserved = readBEword(pHeader->reserved);
    }
    // added for v4
    if (info.version == 4) {
      if (flags & PSID_SIDMODEL3_6581) info.sidModel |= SIDTUNE_SIDMODEL3_6581;
      if (flags & PSID_SIDMODEL3_8580) info.sidModel |= SIDTUNE_SIDMODEL3_8580;
      info.reserved = readBEword(pHeader->reserved);
    }
  ...
  if (info.compatibility != SIDTUNE_COMPATIBILITY_BASIC) {
    if (info.initAddr == 0) info.initAddr = info.loadAddr;
  }
...
bool sidTune::PSID_fileSupportSave(ofstream& fMyOut, const ubyte* dataBuffer) {
  ...
  fMyOut.write((const char*)dataBuffer + fileOffset,
               info.dataFileLen - fileOffset);  // !cast!
