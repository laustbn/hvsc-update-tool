  ...
  if (isCached && status) {
    ...
    if (info.c64dataLen > 65536) {
      ...
      if (endPos <= 65536) {
        ...
        memcpy(c64buf + info.loadAddr, cachePtr + fileOffset,
               info.c64dataLen - (endPos - 65536));
        // Wrap the remaining data to the start address of the C64 memory.
        memcpy(c64buf,
               cachePtr + fileOffset + info.c64dataLen - (endPos - 65536),
               (endPos - 65536));
      }
      return (status = true);
    }
...
udword sidTune::loadFile(const char* fileName, ubyte** bufferRef) {
  ...
  myIn.seekg(0, ios::end);  // explicit ios::ate
                            // As a replacement for !is_open(), bad() and the
                            // NOT-operator don't seem to work on all systems.
...
bool sidTune::cacheRawData(const void* sourceBuf, udword sourceBufLen) {
  ...
  if ((cachePtr = new ubyte[sourceBufLen]) == 0) {
    ...
    if (sourceBufLen >= 2) {
      ...
      info.fixLoad = (readLEword((const ubyte*)sourceBuf + fileOffset) ==
                      (info.loadAddr + 2));
    }
...
bool sidTune::saveToOpenFile(ofstream& toFile, const ubyte* buffer,
                             udword bufLen) {
...
bool sidTune::saveC64dataFile(const char* fileName, bool overWriteFlag) {
  ...
  if (status) {
    ...
    if (!fMyOut) {
      ...
      if (!saveToOpenFile(fMyOut, cachePtr + fileOffset,
...
bool sidTune::checkCompatibility(void) {
  switch (info.compatibility) {
    ...
    case SIDTUNE_COMPATIBILITY_BASIC:
      ...
      if (info.loadAddr < 0x07e8) return false;
      if (info.playAddr != 0) return false;
      if (info.compatibility == SIDTUNE_COMPATIBILITY_R64) break;
      if (info.initAddr != 0) return false;
      break;
  }
  return true;
